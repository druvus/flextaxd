...
class TestNewCLI:
    ...
    def test_export_command_help(self):
        ...
        assert result.returncode == 0
        assert "Export a taxonomy database to classifier formats, createtaxdb formats, or other formats" in result.stdout
        assert "--format" in result.stdout
    ...
    def test_create_basic_database(self, sample_taxonomy_file):
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, "-m", "flextaxd.cli", "create",
    ...
    def test_stats_command_with_database(self, temp_database):
        ...
        result = subprocess.run([
            sys.executable, "-m", "flextaxd.cli", "stats",
    ...
    def test_error_handling_missing_database(self):
        ...
        result = subprocess.run([
            sys.executable, "-m", "flextaxd.cli", "stats",
            "--database", "/nonexistent/database.ftd"
        ], capture_output=True, text=True, timeout=30)
    ...
    def test_verbose_logging(self, temp_database):
        ...
        result = subprocess.run([
            sys.executable, "-m", "flextaxd.cli", 
            "--verbose", "--verbose",
            "stats", "--database", temp_database
        ], capture_output=True, text=True, timeout=30)
