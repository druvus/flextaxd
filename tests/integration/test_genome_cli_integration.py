...
genome_2\t4"""
        ...
        with SQLiteTaxonomyRepository(database_file) as repo:
            ...
            assert (
                stats["node_count"] == 5
            )  # 5 taxonomy nodes (including implicit root)
            assert stats["genome_count"] == 5  # 5 genome mappings

            # Verify specific genome mappings (using auto-generated IDs)
            genomes_for_ecoli = repo.get_genomes_for_node(
                5
            )  # E. coli (auto-generated ID)
            assert len(genomes_for_ecoli) == 3  # seq1, seq2, genome_1

            genomes_for_escherichia = repo.get_genomes_for_node(
                4
            )  # Escherichia genus (auto-generated ID)
...
Escherichia\tEscherichia coli\t562\tspecies"""
        ...
        # Create genome files
        (genomes_dir / "genome1.fasta").write_text(
            ">seq1_chr1\nATCGATCG\n>seq1_plasmid\nGCTAGCTA"
        )
...
Escherichia\tEscherichia coli\t562\tspecies"""
        ...
        (genomes_dir / "ecoli_genome.fasta").write_text(
            """>NC_000913.3 E. coli chromosome
ATCGATCGATCGATCG
>NC_007779.1 E. coli plasmid
GCTAGCTAGCTAGCTA"""
        )

        (genomes_dir / "escherichia_16S.fasta").write_text(
            """>16S_rRNA_gene
GGGGAAAATTTTCCCC"""
        )
