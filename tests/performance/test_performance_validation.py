...
from flextaxd.database.high_performance_sqlite import (
    HighPerformanceTaxonomyDatabase,
    create_high_performance_database,
)
from flextaxd.parsers.high_performance_parser import (
    HighPerformanceNCBIParser,
    create_high_performance_ncbi_parser,
)
...
class TestPerformanceValidation:
    ...
    def test_loading_performance_comparison(self, validation_database):
        ...
        if total_nodes > 100000:
            ...
            memory_reduction = (
                (estimated_std_memory - hp_memory) / estimated_std_memory * 100
            )
            ...
            try:
                ...
                print(
                    f"   🎯 Loading rate: {total_nodes / std_load_time:,.0f} nodes/second"
                )
        ...
        if total_nodes > 10000:  # Only enforce for meaningful datasets
            ...
            ), f"Memory reduction too low: {memory_reduction:.1f}% < 50%"

        print(
            f"   ✅ Performance targets {'MET' if speedup > 5.0 and memory_reduction > 50.0 else 'MISSED'}"
        )
    ...
    def test_query_performance_scaling(self, validation_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            test_pairs = [
                (random.choice(node_ids), random.choice(node_ids)) for _ in range(2000)
            ]
            ...
            print(
                f"   ✅ Success rate: {successful_queries / len(test_pairs) * 100:.1f}%"
            )
            ...
            start_time = time.time()
            batch_results = list(
                tree.batch_lca(test_pairs[:1000], max_workers=mp.cpu_count())
            )
            ...
            print(
                f"   🚀 Parallel speedup: {lca_qps / batch_qps * 1000 / len(test_pairs):.1f}x"
            )
            ...
            if total_nodes > 50000:  # Enforce for larger datasets
                assert (
                    lca_target_met
                ), f"LCA performance below target: {lca_qps:.0f} < {min_lca_qps}"
                assert (
                    path_target_met
                ), f"Path performance below target: {path_qps:.0f} < {min_path_qps}"
    ...
    def test_memory_efficiency_validation(self, validation_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            estimated_standard_bytes = (
                total_nodes * 500
            )  # Conservative estimate for standard Python objects
            compression_ratio = (
                estimated_standard_bytes / stats["estimated_memory_bytes"]
            )

            print(f"\\n🗜️  Compression Analysis:")
            print(
                f"   📈 Estimated standard tree: {estimated_standard_bytes / 1024 / 1024:.1f} MB"
            )
            ...
            print(
                f"   🔢 Bytes per node: {memory_per_node_bytes:.1f} (target: <{max_bytes_per_node})"
            )
            print(
                f"   🗜️  Compression ratio: {compression_ratio:.1f}x (target: >{min_compression_ratio:.1f}x)"
            )
            ...
            print(
                f"   ✅ Compression Target: {'MET' if compression_target_met else 'MISSED'}"
            )

            if total_nodes > 50000:  # Enforce for larger datasets
                assert (
                    bytes_target_met
                ), f"Memory per node too high: {memory_per_node_bytes:.1f} > {max_bytes_per_node}"
                assert (
                    compression_target_met
                ), f"Compression ratio too low: {compression_ratio:.1f}x < {min_compression_ratio}x"
    ...
    def test_comprehensive_performance_benchmark(self, validation_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            benchmark_results = benchmark_performance(
                tree, num_samples=min(5000, total_nodes)
            )
            ...
            print("-" * 40)
            print(
                f"   📊 Score: {performance_score}/{total_tests} ({performance_percentage:.0f}%)"
            )
            ...
            for metric in ["lca_queries_per_second", "path_queries_per_second"]:
                ...
                print(
                    f"   {metric.replace('_', ' ').title()}: {improvement:.1f}x improvement (target: >10x)"
                )
            ...
            memory_reduction = (
                (baseline_memory - actual_memory) / baseline_memory * 100
            ...
            if total_nodes > 100000 and performance_percentage >= 80:
                ...
                pytest.fail(
                    f"Performance targets not met: {performance_percentage:.0f}% < 80%"
                )
