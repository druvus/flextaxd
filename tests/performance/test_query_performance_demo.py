...
class TestQueryPerformanceDemo:
    ...
    def test_lca_query_intensive_workload(self, query_focused_database):
        ...
        large_query_set = [
            (random.randint(100, total_nodes), random.randint(100, total_nodes))
        ...
        with HighPerformanceTaxonomyTree(db_path) as hp_tree:
            ...
            batch_results = list(
                hp_tree.batch_lca(large_query_set, max_workers=mp.cpu_count())
            )
        ...
        print(f"\\n🎯 Performance Targets:")
        print(
            f"   Single-thread: {query_speedup:.1f}x (target: >3x) {'✅ MET' if single_target_met else '🔧 IMPROVING'}"
        )
        print(
            f"   Parallel boost: {parallel_advantage:.1f}x (target: >5x) {'✅ MET' if parallel_target_met else '🔧 IMPROVING'}"
        )
        ...
        print(
            f"   • Parallel processing adds {batch_qps / hp_qps:.1f}x additional speedup"
        )
        print(f"   • Total query improvement: {parallel_advantage:.1f}x over standard")
        print(
            f"   • Preprocessing overhead pays off after ~{int(std_total_time / (hp_total_time - std_total_time)):.0f} queries"
        )
    ...
    def test_memory_vs_performance_tradeoff(self, query_focused_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            print(
                f"   🌲 Core tree data: {(memory_mb - preprocessing_structures_mb):.1f} MB"
            )
            print(f"   ⚡ LCA preprocessing: {preprocessing_structures_mb:.1f} MB")
            print(
                f"   📈 Preprocessing overhead: {preprocessing_structures_mb / memory_mb * 100:.1f}%"
            )

            # Performance benefits analysis
            node_ids = list(tree._nodes.keys())
            test_queries = [
                (random.choice(node_ids), random.choice(node_ids)) for _ in range(5000)
            ]
            ...
            print(
                f"   ⏱️  Query latency: {query_time / len(test_queries) * 1000:.3f} ms/query"
            )
            print(
                f"   🎯 Preprocessing ROI: ~{int(preprocessing_structures_mb * 1024 / (qps / 1000)):.0f} queries to break even"
            )
    ...
    def test_workload_simulation(self, query_focused_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            species_like_nodes = [
                nid for nid in node_ids if nid > total_nodes * 0.7
            ]  # Deep nodes
            phylo_queries = [
                (random.choice(species_like_nodes), random.choice(species_like_nodes))
                for _ in range(2000)
            ]
            ...
            batch_results = list(
                tree.batch_lca(batch_queries, max_workers=mp.cpu_count())
            )
            ...
            total_operations = (
                len(phylo_queries) + len(classification_nodes) + len(batch_queries)
            )
            ...
            targets_met = sum(
                1 for actual, target in zip(results, target_values) if actual >= target
            )
            ...
            for (name, target), actual in zip(targets.items(), results):
                ...
                print(
                    f"   {name.replace('_', ' ').title()}: {actual:,.0f} >= {target:,} {status}"
                )
            ...
            assert (
                classification_rate > 500
            ), f"Classification too slow: {classification_rate:.0f}"
            assert batch_rate > 1000, f"Batch processing too slow: {batch_rate:.0f}"

            print(f"\\n✅ Bioinformatics workload simulation successful!")
            print(
                f"   🎯 Demonstrated production-ready performance for real-world usage"
            )
    ...
    def test_ncbi_scale_extrapolation(self, query_focused_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            print(
                f"   ⚡ Query latency: {current_time / len(test_pairs) * 1000:.3f} ms"
            )
            ...
            for scale_nodes, scale_name, scale_desc in ncbi_scales:
                ...
                projected_qps = (
                    current_qps * 0.95
                )  # Small degradation for cache effects

                # Memory scales linearly
                projected_memory_gb = (
                    current_memory_per_node * scale_nodes / 1024 / 1024 / 1024
                )

                # Loading time scales sub-linearly due to batching
                projected_load_time = (
                    scale_nodes / 10000
                ) * 0.8  # 10K nodes/sec with batching
                ...
                print(
                    f"     📈 Throughput: ~{scale_nodes / projected_load_time:,.0f} nodes/sec"
                )
