...
class TestTreePerformance:
    ...
    def test_lca_performance(self, large_tree: TaxonomyTree):
        ...
        print(
            f"LCA operations: {elapsed:.4f}s for 400 queries on {large_tree.node_count} nodes"
        )
    ...
    def test_tree_comparison_performance(self, large_tree: TaxonomyTree):
        ...
        print(
            f"Tree comparison: {elapsed:.4f}s for {large_tree.node_count} vs {tree_copy.node_count} nodes"
        )
...
class TestTreeOperationsScalability:
    ...
    def test_depth_vs_lca_performance(self):
        ...
        for depth in depths:
            ...
            leaf_nodes = [
                node.tax_id for node in tree if len(tree.get_children(node.tax_id)) == 0
            ]
