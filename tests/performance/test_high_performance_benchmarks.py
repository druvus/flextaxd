...
class TestHighPerformanceBenchmarks:
    ...
    def ncbi_scale_database(self):
        ...
        phylum_ids = []
        phyla_per_sk = [50 * scale_factor, 30 * scale_factor, 40 * scale_factor]  # Realistic distribution
        ...
        for phylum_id in phylum_ids:
            classes_per_phylum = random.randint(5 * scale_factor, 20 * scale_factor)  # Variable
        ...
        order_ids = []
        for class_id in class_ids[:len(class_ids)//2]:  # Not all classes have orders (realistic)
        ...
        family_ids = []
        for order_id in order_ids[:len(order_ids)//3]:  # Not all orders have families
        ...
        print(f"  Species: ~{total_nodes - len(genus_ids) - len(family_ids) - len(order_ids) - len(class_ids) - len(phylum_ids) - len(superkingdom_ids) - 1}")
    ...
    def test_high_performance_vs_standard_loading(self, ncbi_scale_database):
        ...
        print(f"  Memory usage: {hp_stats['estimated_memory_bytes'] / 1024 / 1024:.1f} MB")
        ...
        try:
            if total_nodes > 200000:  # Skip standard tree for very large datasets
                ...
                print(f"  Memory usage: {standard_memory / 1024 / 1024:.1f} MB (estimated)")
        ...
        if standard_load_time > 0:
            ...
            memory_reduction = (standard_memory - hp_stats['estimated_memory_bytes']) / standard_memory * 100
            ...
            if total_nodes > 50000:
                ...
                assert memory_reduction > 30.0, f"Expected >30% memory reduction, got {memory_reduction:.1f}%"
    ...
    def test_lca_performance_scaling(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            # Test 1: Random pairs
            random_pairs = [(random.choice(node_ids), random.choice(node_ids)) for _ in range(1000)]
            ...
            species_nodes = [nid for nid in node_ids if 'Species_' in tree.string_pool.get_string(tree._nodes[nid].name_id)][:500]
            deep_pairs = [(species_nodes[i], species_nodes[i+1]) for i in range(0, len(species_nodes)-1, 2)]
            ...
            if total_nodes > 10000:
                ...
                start_time = time.time()
                batch_results = list(tree.batch_lca(batch_pairs, max_workers=mp.cpu_count()))
                ...
                print(f"  Parallel speedup: {random_lca_time / batch_lca_time * len(batch_pairs) / 1000:.1f}x")
            
            # Performance targets for NCBI-scale
            target_qps = 1000 if total_nodes < 100000 else 500  # Lower target for very large trees
            ...
            assert actual_qps > target_qps, f"LCA performance below target: {actual_qps:.0f} < {target_qps}"
    ...
    def test_memory_efficiency_scaling(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            estimated_standard_memory = total_nodes * 200  # Conservative estimate for standard tree
            compression_ratio = estimated_standard_memory / stats['estimated_memory_bytes']
            
            print(f"  Compression ratio: {compression_ratio:.1f}x vs standard tree")
            
            assert memory_per_node < max_memory_per_node, f"Memory usage too high: {memory_per_node:.1f} > {max_memory_per_node}"
            assert compression_ratio > 3.0, f"Insufficient compression: {compression_ratio:.1f}x < 3.0x"
    ...
    def test_parallel_operations_scaling(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            start_time = time.time()
            distance_matrix = tree.parallel_distance_matrix(test_nodes[:20], max_workers=1)  # Sequential
            sequential_time = time.time() - start_time
            
            start_time = time.time()
            distance_matrix_parallel = tree.parallel_distance_matrix(test_nodes[:20], max_workers=mp.cpu_count())
            ...
            if total_nodes > 1000:
                lca_pairs = [(random.choice(node_ids), random.choice(node_ids)) for _ in range(1000)]
                ...
                lca_speedup = sequential_lca_time / batch_lca_time if batch_lca_time > 0 else 1.0
                ...
                assert seq_dict == batch_dict, "Batch results don't match sequential results"
            
            # Performance targets
            expected_min_speedup = max(1.5, mp.cpu_count() * 0.3)  # At least 30% efficiency
            ...
            if 'batch_lca_time' in locals():
                ...
                assert lca_speedup > expected_min_speedup, f"LCA parallel speedup too low: {lca_speedup:.1f}x"
    ...
    def test_streaming_operations_memory_constant(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            roots = [nid for nid in tree._nodes.keys() if 'Phylum_' in tree.string_pool.get_string(tree._nodes[nid].name_id)]
            if not roots:
            ...
            print(f"  Memory per streamed node: {memory_increase / streamed_count * 1024:.1f} KB")
            
            # Memory should remain roughly constant
            memory_per_node_kb = memory_increase / streamed_count * 1024 if streamed_count > 0 else 0
            
            print(f"\\nStreaming memory assessment:")
            print(f"  Target: <1 KB per streamed node")
            print(f"  Actual: {memory_per_node_kb:.2f} KB per node")
            
            assert memory_per_node_kb < 1.0, f"Streaming uses too much memory: {memory_per_node_kb:.2f} KB/node"
    ...
    def test_ncbi_scale_stress_test(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            print(f"  Memory usage: {final_stats['estimated_memory_bytes'] / 1024 / 1024:.1f} MB")
            ...
            assert ops_per_second > target_ops_per_second, f"Stress test performance: {ops_per_second:.0f} < {target_ops_per_second}"
    ...
    def test_benchmark_comparison(self, ncbi_scale_database):
        ...
        with HighPerformanceTaxonomyTree(db_path) as tree:
            ...
            # Run benchmark
            benchmark_results = benchmark_performance(tree, num_samples=min(2000, total_nodes))
            ...
            for metric, target in targets.items():
                ...
                if 'memory' in metric:
                    met = actual <= target
                    print(f"  {metric}: {actual:.1f} <= {target:.1f} ({'PASS' if met else 'FAIL'})")
                else:
                    met = actual >= target  
                    print(f"  {metric}: {actual:,.0f} >= {target:,.0f} ({'PASS' if met else 'FAIL'})")
