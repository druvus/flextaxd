...
Escherichia	Escherichia coli	562	species"""
        ...
        assert any('Escherichia coli' in line and 'scientific name' in line for line in lines)
        assert any('Escherichia' in line and 'scientific name' in line for line in lines)
        assert any('Proteobacteria' in line and 'scientific name' in line for line in lines)
...
1.2	1.2.1"""
        ...
        with SQLiteTaxonomyRepository(database_file) as repo:
            ...
            assert 'subspecies' in stats['rank_distribution']  # Should have subspecies ranks
        ...
        for fmt in formats_to_test:
            ...
            if output_file.is_dir():
                ...
                if taxonomy_dir.exists():
                    ...
                    names_file = taxonomy_dir / "names.dmp"
                    nodes_file = taxonomy_dir / "nodes.dmp"
                    assert names_file.exists() or nodes_file.exists(), f"No taxonomy files found in {fmt} output"
                else:
                    ...
                    assert len(content_files) > 0, f"No content files found in {fmt} directory"
            else:
        ...
        with SQLiteTaxonomyRepository(database_file) as repo:
            ...
            if stats['rank_distribution']:
                for rank, expected_count in expected_ranks.items():
                    ...
                    assert actual_count == expected_count, f"Expected {expected_count} {rank} nodes, got {actual_count}"