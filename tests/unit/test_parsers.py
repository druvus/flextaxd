...
from flextaxd.parsers import (
    NCBITaxonomyParser, TSVTaxonomyParser, QIIMETaxonomyParser,
...
class TestQIIMETaxonomyParser:
    ...
    def test_parse_qiime_taxonomy_string(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as f:
            f.write("OTU_1\tk__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Enterobacterales;f__Enterobacteriaceae;g__Escherichia;s__Escherichia_coli\n")
            f.flush()
            test_file = Path(f.name)
...
class TestParserRegistry:
    ...
    def test_register_invalid_parser(self):
        ...
        with pytest.raises(ConfigurationError, match="must inherit from TaxonomyParser"):
        ...
        try:
            for parser in parsers:
                ...
                try:
                    ...
                    pytest.fail(f"Parser {parser.parser_name} raised unexpected exception: {e}")
