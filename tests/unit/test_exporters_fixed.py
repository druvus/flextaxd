...
from .test_fixtures import (
    ...
    complex_taxonomy_tree,
    tree_with_genomes,
    mock_sequence_files,
    mock_genome_directory,
    expected_ncbi_formats,
)
...
class TestNCBIExporter:
    ...
    def test_export_basic_functionality(
        self, simple_taxonomy_tree, expected_ncbi_formats
    ):
...
class TestGenomeSizesExporter:
    ...
    def test_export_with_mock_sizes(self, tree_with_genomes):
        ...
        try:
            ...
            assert any(
                col in header for col in ["taxid", "accession", "size", "length"]
            )
...
class TestSylphExporter:
    ...
    def test_export_compression_options(self, simple_taxonomy_tree):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_path = Path(tmp_dir)
            exporter.export(
                simple_taxonomy_tree, output_path, compress=True, compression_level=300
            )
