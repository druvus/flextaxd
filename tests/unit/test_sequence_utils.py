...
class TestSeqIDMapping:
    ...
    def test_create_mapping_with_source(self):
        ...
        mapping = SeqIDMapping(
            sequence_id="GCF_123456.1", taxonomy_id=562, source="NCBI"
        )
...
"""
    ...
    def test_extract_gtdb_taxonomy_from_header(self):
        ...
        header = (
            ">GB_GCA_000001405.38 d__Bacteria;p__Proteobacteria;c__Gammaproteobacteria"
        )
        ...
        header = (
            ">RS_GCF_000005825.2 d__Bacteria;p__Firmicutes;c__Bacilli;o__Bacillales"
        )
        result = processor.extract_gtdb_taxonomy_from_header(header)
...
"""
    ...
    def test_get_statistics_populated(self):
        ...
        manager.mappings["seq3"] = SeqIDMapping(
            "seq3", 456, "source2"
        )  # Different tax ID and source
...
class TestGenomeDirectoryManager:
    ...
    def test_scan_directory_with_genome_files(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            gzipped_found = any(
                "genome4" in genome_id for genome_id in manager.genome_files.keys()
            )
            assert (
                gzipped_found
            ), f"Expected genome4 in {list(manager.genome_files.keys())}"
    ...
    def test_get_genome_statistics(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            (temp_path / "genome1.fasta").write_text(
                ">seq1\nATCGATCGATCG" * 100
            )  # Longer content
            (temp_path / "genome2.fa").write_text(">seq2\nGCTA" * 50)
            ...
            )  # Allow 0.0 due to rounding for small files
