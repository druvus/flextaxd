...
class TestSQLiteTaxonomyRepository:
    ...
    def test_add_node_invalid_parent(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                with pytest.raises(DatabaseError, match="Parent node .* does not exist"):
                    repo.add_node(node)
        finally:
            if db_path.exists():
                db_path.unlink()
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                nodes = [
                    ...
                    TaxonomyNode(tax_id=562, name="E. coli", rank=TaxonomicRank.SPECIES, parent_id=2)
                ]
    ...
    def test_get_children(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                salmonella = TaxonomyNode(tax_id=590, name="Salmonella", rank=TaxonomicRank.GENUS, parent_id=2)
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                nodes = [
                    TaxonomyNode(tax_id=1, name="root", rank=TaxonomicRank.CUSTOM),
                    TaxonomyNode(tax_id=2, name="Bacteria", rank=TaxonomicRank.SUPERKINGDOM, parent_id=1),
                    TaxonomyNode(tax_id=561, name="Escherichia", rank=TaxonomicRank.GENUS, parent_id=2),
                    TaxonomyNode(tax_id=562, name="E. coli", rank=TaxonomicRank.SPECIES, parent_id=561),
                    TaxonomyNode(tax_id=1000, name="K-12", rank=TaxonomicRank.STRAIN, parent_id=562)
                ]
    ...
    def test_delete_node_leaf(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                root = TaxonomyNode(tax_id=1, name="root", rank=TaxonomicRank.CUSTOM)
                leaf = TaxonomyNode(tax_id=2, name="leaf", rank=TaxonomicRank.SPECIES, parent_id=1)
    ...
    def test_delete_node_with_children_fails(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                root = TaxonomyNode(tax_id=1, name="root", rank=TaxonomicRank.CUSTOM)
                child = TaxonomyNode(tax_id=2, name="child", rank=TaxonomicRank.SPECIES, parent_id=1)
    ...
    def test_add_genome_basic(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                ecoli = TaxonomyNode(tax_id=562, name="E. coli", rank=TaxonomicRank.SPECIES)
                repo.add_node(ecoli)
    ...
    def test_get_statistics_populated_database(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                nodes = [
                    TaxonomyNode(tax_id=1, name="root", rank=TaxonomicRank.CUSTOM),  # Root
                    TaxonomyNode(tax_id=2, name="Bacteria", rank=TaxonomicRank.SUPERKINGDOM, parent_id=1),
                    TaxonomyNode(tax_id=562, name="E. coli", rank=TaxonomicRank.SPECIES, parent_id=2),  # Leaf
                    TaxonomyNode(tax_id=590, name="Salmonella", rank=TaxonomicRank.SPECIES, parent_id=2)  # Leaf
                ]
            ...
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                with pytest.raises(DatabaseError):
                    with repo.transaction():
                        ...
                        invalid_node = TaxonomyNode(tax_id=3, name="Invalid", rank=TaxonomicRank.SPECIES, parent_id=999)
    ...
    def test_disk_space_simulation(self):
        ...
        try:
            with SQLiteTaxonomyRepository(db_path) as repo:
                ...
                with patch.object(repo._get_connection(), 'execute', side_effect=sqlite3.OperationalError("database or disk is full")):
