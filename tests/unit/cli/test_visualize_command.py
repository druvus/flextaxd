...
class TestVisualizeCommand:
    ...
    @patch("flextaxd.cli.commands.visualize.SQLiteTaxonomyRepository")
    def test_tree_visualization_with_depth_limit(
        self, mock_repo_class, simple_tree, capsys
    ):
    ...
    @patch("flextaxd.cli.commands.visualize.SQLiteTaxonomyRepository")
    def test_tree_visualization_from_specific_node(
        self, mock_repo_class, simple_tree, capsys
    ):
        ...
        tree_lines = [
            line for line in lines if any(char in line for char in ["├", "└", "│"])
        ]
...
class TestAdvancedVisualization:
    ...
    def test_newick_format_output(self, mock_repo_class, simple_tree, capsys):
        ...
        assert "Bacteria" in newick_output or "Bacteria" in newick_output.replace(
            "_", " "
        )
        assert "Escherichia" in newick_output or "Escherichia" in newick_output.replace(
            "_", " "
        )
    ...
    def test_newick_string_generation(self, simple_tree):
        ...
        assert "Bacteria" in newick_limited or "Bacteria" in newick_limited.replace(
            "_", " "
        )
    ...
    def test_newick_special_character_handling(self):
        ...
        assert "(" not in newick or newick.count("(") == newick.count(
            ")"
        )  # Balanced parentheses only for tree structure
    ...
    def test_newick_vis_missing_biopython(self, mock_repo_class, simple_tree):
        ...
        with patch.object(command, "_validate_database_path"):
            with patch(
                "builtins.__import__", side_effect=ImportError("No module named 'Bio'")
            ):
                result = command.execute(MockArgs())
