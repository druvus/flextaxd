...
class TestMMseqs2Exporter:
    ...
    def test_genetic_code_assignment(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            bacteria_line = [line for line in content.split('\n') if 'Bacteria' in line and '2\t|' in line][0]
    ...
    def test_mmseqs2_accession_extraction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            accession_lines = [line for line in lines if not line.startswith('accession')]
    ...
    def _create_tree_with_complex_accessions(self):
        ...
        genome = GenomeInfo(
            genome_id="NC_000913.3|GCF_000005825.2",
            tax_id=562,
            sequence_type="genome"
        )
        tree.add_genome(genome)
