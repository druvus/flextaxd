...
class TestTreeMerging:
    ...
    def test_merge_trees_graft_at_root(self, base_tree, subtree_to_graft):
        ...
        id_mapping = base_tree.merge_trees(subtree_to_graft, merge_strategy='graft_at_root')
    ...
    def test_merge_trees_common_ancestors(self, base_tree):
        ...
        id_mapping = base_tree.merge_trees(other_tree, merge_strategy='merge_common_ancestors')
...
class TestTreeComparison:
    ...
    def test_compare_structural_differences(self):
        ...
        for tree in [tree1, tree2]:
            ...
            proteo = TaxonomyNode(tax_id=3, name="Proteobacteria", rank=TaxonomicRank.PHYLUM, 
                                parent_id=2 if tree is tree1 else 1)  # Different parent!
...
class TestTreeStatistics:
    ...
    def test_branching_factor_stats(self, balanced_tree):
        ...
        assert branching_stats['max'] == 2  # Root and superkingdom nodes have 2 children
        assert branching_stats['min'] == 0  # Leaf nodes have 0 children
        assert branching_stats['average'] == 6/7  # Total children (2+2+2+0+0+0+0) / total nodes
...
class TestAdvancedTreeIntegration:
    ...
    def test_extract_and_graft_workflow(self):
        ...
        new_node = TaxonomyNode(tax_id=999, name="E.coli", rank=TaxonomicRank.SPECIES, parent_id=2)
    ...
    def test_tree_comparison_and_statistics(self):
        ...
        for i, tree in enumerate([tree1, tree2], 1):
            ...
            phylum_id = 10 + i
            phylum = TaxonomyNode(tax_id=phylum_id, name=f"Phylum{i}", rank=TaxonomicRank.PHYLUM, parent_id=2)
