...
class TestSetupLogging:
    ...
    @patch("flextaxd.utils.logging_config.logging.getLogger")
    @patch("flextaxd.utils.logging_config.logging.config.dictConfig")
    def test_setup_logging_no_file_debug_message(
        self, mock_dict_config, mock_get_logger
    ):
...
class TestLoggingIntegration:
    ...
    def test_external_library_noise_reduction(self):
        ...
        assert urllib3_logger.level >= logging.WARNING or urllib3_logger.isEnabledFor(
            logging.WARNING
        )
        assert requests_logger.level >= logging.WARNING or requests_logger.isEnabledFor(
            logging.WARNING
        )
