...
sequence_types = st.one_of(
    st.none(), st.sampled_from(["genome", "protein", "rna", "plasmid", "chromosome"])
)
...
def valid_taxonomy_nodes(draw, parent_id=None):
    ...
    if parent_id is not None:
        ...
        node_parent_id = draw(
            st.one_of(st.none(), valid_tax_ids.filter(lambda x: x != tax_id))
        )

    return TaxonomyNode(tax_id=tax_id, name=name, rank=rank, parent_id=node_parent_id)
...
def valid_genome_info(draw):
    ...
    return GenomeInfo(genome_id=genome_id, tax_id=tax_id, sequence_type=sequence_type)
...
def valid_taxonomy_trees(draw, max_nodes=10):
    ...
    for _ in range(num_additional):
        ...
        new_node = TaxonomyNode(
            tax_id=new_tax_id, name=name, rank=rank, parent_id=parent_id
        )
...
class TestTaxonomyNodeProperties:
    ...
    def test_invalid_tax_id_raises_error(
        self, bad_tax_id: int, name: str, rank: TaxonomicRank
    ):
    ...
    @given(
        valid_tax_ids,
        st.text(max_size=0).filter(lambda x: not x.strip()),
        taxonomic_ranks,
    )
    @settings(max_examples=50)
    def test_empty_name_raises_error(
        self, tax_id: int, empty_name: str, rank: TaxonomicRank
    ):
    ...
    @settings(max_examples=50)
    def test_invalid_parent_id_raises_error(
        self, tax_id: int, name: str, rank: TaxonomicRank, bad_parent_id: int
    ):
    ...
    @settings(max_examples=50)
    def test_self_parent_raises_error(
        self, tax_id: int, name: str, rank: TaxonomicRank
    ):
...
class TestGenomeInfoProperties:
    ...
    @settings(max_examples=50)
    def test_invalid_tax_id_in_genome_raises_error(
        self, genome_id: str, bad_tax_id: int
    ):
...
class TestTaxonomyTreeProperties:
    ...
    @settings(max_examples=50)
    def test_adding_duplicate_node_raises_error(
        self, tree: TaxonomyTree, new_node: TaxonomyNode
    ):
        ...
        if not issues:
            ...
            for node in tree:
                if node.parent_id is not None:
                    ...
                    assert (
                        parent is not None
                    ), f"Parent {node.parent_id} not found for node {node.tax_id}"

    @given(valid_taxonomy_trees(max_nodes=3), valid_genome_info())
    @settings(max_examples=30)
    def test_adding_genome_to_existing_node_succeeds(
        self, tree: TaxonomyTree, genome: GenomeInfo
    ):
    ...
    def test_tsv_parser_handles_arbitrary_input(self, random_text: str):
        ...
        try:
            ...
            try:
                ...
                assert isinstance(e, (ValueError, RuntimeError)) or "Parse" in str(
                    type(e)
                )
        finally:
            if test_file.exists():
...
class TestMemoryAndPerformance:
    ...
    def test_large_tree_memory_usage(self, num_nodes: int):
        ...
        for i in range(2, num_nodes + 2):
            node = TaxonomyNode(
                tax_id=i, name=f"node_{i}", rank=TaxonomicRank.SPECIES, parent_id=1
            )
            tree.add_node(node)
