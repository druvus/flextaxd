...
class HighPerformanceTaxonomyDatabase:
    ...
    def bulk_insert_nodes(
        ...
        with self.pool.get_connection() as conn:
            ...
            for i in range(0, len(nodes), batch_size):
                ...
                cursor.executemany(
                    self._prepared_statements["bulk_insert_nodes"], batch_data
                )
    ...
    def parallel_bulk_insert_nodes(
        self, nodes: List[TaxonomyNode], max_workers: int = 4
    ) -> int:
    ...
    def _insert_node_chunk(self, nodes: List[TaxonomyNode], chunk_id: int) -> int:
        ...
        with tempfile.NamedTemporaryFile(
            suffix=f"_chunk_{chunk_id}.db", delete=False
        ) as tmp_f:
    ...
    def get_node_with_lineage(
        self, tax_id: int
    ) -> Optional[Tuple[TaxonomyNode, List[str]]]:
        ...
        with self.pool.get_connection() as conn:
            ...
            node = TaxonomyNode(
                tax_id=tax_id, name=name, rank=taxonomic_rank, parent_id=parent_id
            )
    ...
    def fast_lca_query(self, tax_id1: int, tax_id2: int) -> Optional[int]:
        ...
        with self.pool.get_connection() as conn:
            ...
            if lca_name:
                # Find tax_id for LCA
                cursor.execute(
                    "SELECT tax_id FROM nodes WHERE name = ? LIMIT 1", (lca_name,)
                )
...
def create_high_performance_database(
    db_path: str, max_connections: int = 32
