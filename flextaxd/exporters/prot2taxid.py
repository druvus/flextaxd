...
class Prot2TaxidExporter(FileBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Prot2Taxid format: {output_path}")
        
        # Configuration options
        compress = kwargs.get('compress', False)
        include_genomes = kwargs.get('include_genomes', True)
        sequence_filter = kwargs.get('sequence_filter', 'protein')  # protein, nucleotide, or all
    ...
    def _write_prot2taxid_file(self, tree: TaxonomyTree, output_path: Path, 
