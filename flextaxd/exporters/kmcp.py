...
class KMCPExporter(FileBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to KMCP mapping format: {output_path}")
        
        # Configuration options
        compress = kwargs.get('compress', False)
        include_genomes = kwargs.get('include_genomes', True)
        sequence_type = kwargs.get('sequence_type', None)
        include_header = kwargs.get('header', False)  # KMCP typically doesn't use headers
        ...
        self._write_kmcp_mapping_file(tree, output_path, compress, sequence_type, include_header)
        
        logger.info(f"KMCP mapping export completed: {output_path}")
    
    def _write_empty_file(self, output_path: Path, compress: bool, include_header: bool) -> None:
    ...
    def _write_kmcp_mapping_file(self, tree: TaxonomyTree, output_path: Path, compress: bool, 
                                 sequence_type: Optional[str] = None, include_header: bool = False) -> None:
        ...
        with open_func(actual_path, 'wt', encoding='utf-8') as f:
            ...
            for node in tree:
                ...
                for genome in genomes:
                    ...
                    if sequence_type is not None and genome.sequence_type != sequence_type:
                        continue
