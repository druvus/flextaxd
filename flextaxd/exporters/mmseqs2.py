...
class MMseqs2Exporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(
            f"Exporting {tree.node_count} nodes to MMseqs2 format: {output_path}"
        )
        ...
        if include_genomes and tree.genome_count > 0:
            ...
            if create_lca_mapping:
                ...
                self._write_taxonomy_mapping(
                    tree, mapping_path, compress, sequence_type
                )
    ...
    def _write_accession_mapping(
        self, tree: TaxonomyTree, output_path: Path, compress: bool, sequence_type: str
    ) -> None:
        ...
        logger.info(
            f"Created MMseqs2 accession mapping with {sequences_mapped} entries"
        )
        ...
        with open(output_path, "w", encoding="utf-8", newline="\n") as f:
            ...
            for node in tree:
                ...
                for genome in genomes:
                    ...
                    # Write mapping entry
                    f.write(
                        f"{genome.genome_id}\t{node.tax_id}\t{lineage}\t{rank}\t{lca_level}\n"
                    )
                    mappings_created += 1
                    ...
                    for acc in additional_accs:
                        f.write(
                            f"{acc}\t{node.tax_id}\t{lineage}\t{rank}\t{lca_level}\n"
                        )
                        mappings_created += 1
    ...
    def _write_mmseqs2_config(self, tree: TaxonomyTree, output_path: Path) -> None:
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                "#    mmseqs createtaxdb seqDB taxDB --tax-mapping-file accession2taxid.txt \\\n"
            )
    ...
    def _should_add_common_name(self, scientific_name: str) -> bool:
        ...
        return any(
            indicator in scientific_name.lower() for indicator in common_indicators
        )
    ...
    def _validate_parent_id(self, node: Any, tree: TaxonomyTree) -> int:
        ...
        if tree.get_node(node.parent_id) is None:
            logger.warning(
                f"Node {node.tax_id} references non-existent parent {node.parent_id}"
            )
            return int(node.tax_id)
