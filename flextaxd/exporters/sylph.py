...
class SylphExporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        try:
            ...
            logger.info(f"To build Sylph database, run: sylph sketch -l {genome_list_path} -o database -t 50 -c {compression_level}")
    ...
    def _write_genome_files(self, tree: TaxonomyTree, genomes_dir: Path, compress: bool) -> list[str]:
    ...
    def _write_genome_fasta(self, genome: Any, output_path: Path, compress: bool) -> None:
                ...
                for node in tree:
                    ...
                    if tree.genome_count > 0:
                        ...
                        for genome in genomes:
                            ...
                            f.write(f"{genome.genome_id}\t{node.tax_id}\t{escaped_name}\t{rank}\t{lineage}\n")
                    else:
                        # Write placeholder entry for nodes without genomes
                        f.write(f"placeholder_{node.tax_id}\t{node.tax_id}\t{escaped_name}\t{rank}\t{lineage}\n")
    ...
    def _write_sylph_config(self, output_path: Path, compression_level: int, genome_count: int) -> None:
        ...
        try:
            with open(output_path, 'w', encoding='utf-8', newline='\n') as f:
                ...
                f.write(f"# sylph sketch -l genome_list.txt -o database -t 50 -c {compression_level}\n\n")
                
                f.write("# Parameters explanation:\n")
                f.write(f"# -c {compression_level}: Compression parameter (higher = faster but less sensitive)\n")
                f.write("# -t 50: Number of threads\n")
                f.write("# -o database: Output database prefix (creates database.syldb)\n\n")
                ...
                f.write("# For dereplicated genomes, can use higher compression (c=300)\n")
            
            logger.info(f"Created sylph_config.txt with recommended parameters")
            
        except Exception as e:
            logger.error(f"Failed to write sylph_config.txt: {str(e)}")
            raise ExportError(f"Failed to create sylph_config.txt file: {str(e)}") from e
