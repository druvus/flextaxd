...
class Accession2TaxidExporter(FileBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Accession2Taxid format: {output_path}")
    ...
    def _write_accession2taxid_file(self, tree: TaxonomyTree, output_path: Path, compress: bool, sequence_type: Optional[str] = None) -> None:
    ...
    def _extract_accession(self, genome_id: str) -> str:
        ...
        if '.' in genome_id and (genome_id.startswith('GCF_') or genome_id.startswith('GCA_')):
