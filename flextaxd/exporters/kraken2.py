...
class Kraken2Exporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Kraken2 format: {output_path}")
        ...
        logger.info(f"Kraken2 export completed: {names_path}, {nodes_path}, {taxonomy_tab_path}")
    ...
    def _write_taxonomy_tab(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
    ...
    def _write_seqid_mapping(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
            ...
            for node in tree:
                ...
                for genome in genomes:
                    ...
                    if genome.has_sequence_file and genome.file_path:
                        try:
                            sequence_infos = processor.parse_fasta_headers(Path(genome.file_path))
                            ...
                            logger.warning(f"Could not process FASTA file {genome.file_path}: {e}")
        
        logger.info(f"Created seqid2taxid mapping with {sequences_mapped} sequence entries")
