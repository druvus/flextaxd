...
class GenomeSizesExporter(FileBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Genome Sizes format: {output_path}")
        
        # Configuration options
        compress = kwargs.get('compress', False)
        include_genomes = kwargs.get('include_genomes', True)
        default_size = kwargs.get('default_genome_size', 0)  # Default genome size if not available
    ...
    def _write_genome_sizes_file(self, tree: TaxonomyTree, output_path: Path, 
                                compress: bool, default_size: int) -> None:
        """Write genome sizes mapping file."""
        open_func = self._get_open_function(output_path, compress)
        
        with open_func(output_path, 'wt', encoding='utf-8') as f:
            # Write header
            f.write("taxid\tassembly_accession\tspecies_taxid\torganism_name\tassembly_level\tgenome_size\n")
            ...
            for node in tree:
                ...
                for genome in genomes:
                    ...
                    f.write(f"{taxid}\t{assembly_accession}\t{species_taxid}\t{organism_name}\t{assembly_level}\t{genome_size}\n")
                    entries_written += 1
    ...
    def _get_organism_name(self, node: Any, tree: TaxonomyTree) -> str:
        ...
        return str(node.name).replace('\t', ' ')  # Clean any tabs that might break format
    ...
    def _get_open_function(self, output_path: Path, compress: bool) -> Callable[..., Any]:
