...
class MetabuliExporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Metabuli format: {output_path}")
        ...
        if include_genomes and tree.genome_count > 0:
            ...
            logger.info(f"Created accession2taxid mapping with {tree.genome_count} genome entries")
    ...
    def _write_merged_file(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
    ...
    def _write_accession_mapping(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
        ...
        with open(output_path, 'w', encoding='utf-8', newline='\n') as f:
            ...
            for node in tree:
                ...
                for genome in genomes:
                    ...
                    additional_accessions = self._extract_additional_accessions(genome.genome_id)
                    for acc in additional_accessions:
                        f.write(f"{acc}\t{acc}\t{node.tax_id}\t0\n")
                        sequences_mapped += 1
        
        logger.info(f"Created accession2taxid mapping with {sequences_mapped} accession entries")
                ...
                try:
                    with open(nodes_file, 'r') as f:
                        ...
                        if not first_line or first_line.count('\t|\t') < 11:
                            validation_results['errors'].append("nodes.dmp: Invalid format")
                except Exception as e:
