...
class Ganon2Exporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        try:
            ...
            self._write_input_files(tree, input_files_path, compress, level, include_genomes)
            ...
            logger.info(f"To build Ganon2 database, run: ganon build-custom --input-file {input_files_path}")
    ...
    def _write_input_files(self, tree: TaxonomyTree, output_path: Path, compress: bool, 
                          level: str, include_genomes: bool) -> None:
        ...
        try:
                ...
                for node in tree:
                    ...
                    if include_genomes and tree.genome_count > 0:
                        ...
                        for genome in genomes:
                            ...
                            specialization_name = self._get_specialization_name(genome, escaped_name)
                            
                            # Write entry
                            f.write(f"{file_path}\t{target}\t{node.tax_id}\t{specialization}\t{specialization_name}\n")
                            entry_count += 1
                    else:
            ...
            logger.info(f"Created input_files.txt with {entry_count} entries for Ganon2")
    ...
    def _write_taxonomy_tax(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
        ...
        try:
            with open(output_path, 'w', encoding='utf-8', newline='\n') as f:
                ...
                for node in tree:
                    ...
                    f.write(f"{node.tax_id}\t{parent_id}\t{rank}\t{escaped_name}\t{genome_size}\n")
            
            logger.info(f"Created taxonomy.tax with {tree.node_count} entries for Ganon2")
    ...
    def _write_seq_info_file(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
            ...
            with open(output_path, 'w', encoding='utf-8', newline='\n') as f:
                ...
                for node in tree:
                    ...
                    for genome in genomes:
                        ...
                        f.write(f"{genome.genome_id}\t{seq_length}\t{node.tax_id}\t{assembly_id}\n")
                        genome_count += 1
            
            logger.info(f"Created seq-info.txt with {genome_count} genome entries for Ganon2")
