...
class CentrifugeExporter(DirectoryBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(f"Exporting {tree.node_count} nodes to Centrifuge format: {output_path}")
        ...
        logger.info(f"Centrifuge export completed: {taxonomy_dir}, {names_txt_path}, {nodes_txt_path}")
    ...
    def _write_nodes_file(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
        ...
        with open(output_path, 'w') as f:
            for node in tree:
                ...
                f.write(f"{node.tax_id}\t|\t{parent_id}\t|\t{node.rank.value}\t|\t\t|\t0\t|\t1\t|\t1\t|\t1\t|\t1\t|\t1\t|\t1\t|\t1\t|\t\t|\n")
        
        if compress:
            from ..utils.subprocess_utils import compress_file
            compress_file(output_path)
    
    def _write_names_txt(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
    ...
    def _write_nodes_txt(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
    ...
    def _write_conversion_table(self, tree: TaxonomyTree, output_path: Path, compress: bool) -> None:
