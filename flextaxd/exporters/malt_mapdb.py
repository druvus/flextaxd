...
class MALTMapDBExporter(FileBasedExporter):
    ...
    def export(self, tree: TaxonomyTree, output_path: Path, **kwargs: Any) -> None:
        ...
        logger.info(
            f"Exporting {tree.node_count} nodes to MALT MapDB format: {output_path}"
        )
    ...
    def _create_empty_database(
        self, output_path: Path, db_version: str, db_type: str
    ) -> None:
    ...
    def _create_malt_database(
        self, tree: TaxonomyTree, output_path: Path, db_version: str, db_type: str
    ) -> None:
        ...
        try:
            with sqlite3.connect(output_path) as conn:
                ...
                total_mappings = sum(
                    len(tree.get_genomes_for_node(node.tax_id)) for node in tree
                )
    ...
    def _insert_info(
        ...
        conn.executemany(
            "INSERT OR REPLACE INTO info (key, value) VALUES (?, ?)", info_data
        )
