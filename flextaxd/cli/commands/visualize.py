...
Examples:
  ...
  flextaxd visualize --database my_db.ftd --type plot --output plot.png --label-size 12 --clip-labels
        ...
        parser.add_argument(
            "--show-ids", action="store_true", help="Show node taxonomy IDs"
        )
        ...
        parser.add_argument(
            "--show-ranks", action="store_true", help="Show taxonomic ranks"
        )

        parser.add_argument(
            "--compact", action="store_true", help="Use compact tree display"
        )
    ...
    def _find_start_node(
        self, tree: TaxonomyTree, start_identifier: str
    ...
    def _output_tree_json(
        ...
        def build_tree_dict(
            node: TaxonomyNode, current_depth: int = 0
        ) -> Dict[str, Any]:
    ...
    def _output_summary_text(
        ...
        if args.show_genomes:
            total_genomes = self._count_subtree_genomes(
                tree, start_node, args.max_depth
            )
        ...
        rank_counts = self._get_subtree_rank_distribution(
            tree, start_node, args.max_depth
        )
        if rank_counts:
            ...
            for rank, count in sorted(
                rank_counts.items(), key=lambda x: x[1], reverse=True
            ):
                print(f"  {rank}: {count}")
    ...
    def _count_subtree_nodes(
        ...
        for child_id in children:
            ...
            if child:
                count += self._count_subtree_nodes(
                    tree, child, max_depth, current_depth + 1
                )

        return count

    def _count_subtree_genomes(
        self,
        tree: TaxonomyTree,
        node: TaxonomyNode,
        max_depth: int,
        ...
        for child_id in children:
            child = tree.get_node(child_id)
            if child:
                count += self._count_subtree_genomes(
                    tree, child, max_depth, current_depth + 1
                )

        return count

    def _get_subtree_rank_distribution(
        self,
        tree: TaxonomyTree,
        node: TaxonomyNode,
        max_depth: int,
        ...
        for child_id in children:
            child = tree.get_node(child_id)
            if child:
                child_ranks = self._get_subtree_rank_distribution(
                    tree, child, max_depth, current_depth + 1
                )
    ...
    def _visualize_newick_ascii(
        ...
        try:
            ...
            raise ValidationError(
                "BioPython is required for newick_vis visualization. Install with: pip install biopython"
            )
    ...
    def _visualize_plot(self, tree: TaxonomyTree, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            raise ValidationError(
                f"{missing_dep} is required for plot visualization. Install with: pip install {missing_dep}"
            )
        ...
        try:
            ...
            # Set title
            plt.title(
                f"Taxonomy Tree - Starting from: {start_node.name}",
                fontsize=14,
                fontweight="bold",
            )
            plt.tight_layout()
    ...
    def _generate_newick_string(
        self,
        tree: TaxonomyTree,
        start_node: TaxonomyNode,
        max_depth: int,
        current_depth: int = 0,
    ) -> str:
        ...
        for child_id in children:
            child_node = tree.get_node(child_id)
            if child_node:
                child_newick = self._generate_newick_string(
                    tree, child_node, max_depth, current_depth + 1
                )
