...
Examples:
  ...
  flextaxd export --database my_db.ftd --format newick --output tree.nwk
    ...
    def _export_tsv(self, tree: TaxonomyTree, output_path: Path, args: argparse.Namespace) -> None:
        ...
        with open(output_path, 'w') as f:
            ...
            for node in tree:
                if node.parent_id is not None:
                    ...
                    parent_name = parent_node.name if parent_node else str(node.parent_id)
                    f.write(f"{parent_name}{args.separator}{node.name}{args.separator}{node.rank.value}\n")
        
        if args.compress:
            from ...utils.subprocess_utils import compress_file
            compress_file(output_path)
    
    def _export_newick(self, tree: TaxonomyTree, output_path: Path, args: argparse.Namespace) -> None:
    ...
    def _export_json(self, tree: TaxonomyTree, output_path: Path, args: argparse.Namespace) -> None:
