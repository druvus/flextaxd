...
class TSVTaxonomyParser(FileBasedParser):
    ...
    def parse(self, file_path: Path, **kwargs: Any) -> TaxonomyTree:
        ...
        if has_header and lines:
            ...
            if header_info:
                parent_col = header_info.get('parent', header_info.get('parent_name', parent_col))
                child_col = header_info.get('child', header_info.get('child_name', child_col))
        ...
        for line_num, line in enumerate(lines[start_line:], start=start_line + 1):
            ...
            fields = line.split('\t')
            if len(fields) < 2:
                raise ParseError(f"Line {line_num}: Expected at least 2 columns, got {len(fields)}")
