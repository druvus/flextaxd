...
class HighPerformanceNCBIParser:
    ...
    def parse_nodes_file(
        self, nodes_file: Path, use_parallel: bool = True, chunk_size_mb: int = 100
    ) -> Iterator[ParsedNode]:
        ...
        if (
            not use_parallel
            or os.path.getsize(nodes_file) < chunk_size_mb * 1024 * 1024
        ):
    ...
    def _parse_nodes_parallel(
        self, nodes_file: Path, chunk_size_mb: int
    ) -> Iterator[ParsedNode]:
    ...
    def _parse_compressed_parallel(
        self, file_path: Path, open_func: Any
    ) -> Iterator[ParsedNode]:
    ...
    def parse_names_file(
        self, names_file: Path, name_class: str = "scientific name"
    ) -> Dict[int, str]:
    ...
    def parse_complete_ncbi_taxonomy(
        self, nodes_file: Path, names_file: Path, use_parallel: bool = True
    ) -> Iterator[TaxonomyNode]:
        ...
        for parsed_node in self.parse_nodes_file(nodes_file, use_parallel):
            ...
            try:
                rank = (
                    TaxonomicRank(parsed_node.rank)
                    if parsed_node.rank
...
class HighPerformanceGenomeParser:
    ...
    def parse_genome_assembly_summary(
        self, summary_file: Path
    ) -> Iterator[ParsedGenome]:
...
class ParallelFileProcessor:
    ...
    def process_files_parallel(
        self, file_paths: List[Path], processor_func: Any, use_processes: bool = False
    ) -> Iterator[Any]:
        ...
        with ExecutorClass(max_workers=self.max_workers) as executor:
            futures = [
                executor.submit(processor_func, file_path) for file_path in file_paths
            ]
    ...
    def stream_process_large_file(
        self, file_path: Path, line_processor: Any, batch_size: int = 10000
    ) -> Iterator[Any]:
...
def create_high_performance_ncbi_parser(
...
def benchmark_parser_performance(
    parser: HighPerformanceNCBIParser, test_file: Path, use_parallel: bool = True
) -> Dict[str, Any]:
