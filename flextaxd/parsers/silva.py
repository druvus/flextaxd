...
class SILVATaxonomyParser(FileBasedParser):
    ...
    def _parse_silva_level(
        self, level: str, position: int
    ) -> tuple[TaxonomicRank, str]:
        ...
        level = re.sub(
            r"^[A-Z][a-z]*:", "", level
        )  # Remove rank prefixes like "Domain:"

        # SILVA rank detection based on common patterns
        level_lower = level.lower()

        if any(
            domain in level_lower for domain in ["bacteria", "archaea", "eukaryota"]
        ):
