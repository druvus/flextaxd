...
class GTDBTaxonomyParser(FileBasedParser):
    ...
    def _is_standard_gtdb_taxonomy(self, lines: List[str]) -> bool:
        ...
        for line in lines[:5]:  # Check first few lines
            ...
            if self._is_gtdb_genome_id(genome_id) and self._has_gtdb_taxonomy_string(
                taxonomy
            ):
                return True
    ...
    def _is_gtdb_genome_id(self, genome_id: str) -> bool:
        ...
        if (
            len(genome_id) >= 3
            and not " " in genome_id
            and ("_" in genome_id or "." in genome_id)
        ):
            return True
    ...
    def parse(self, file_path: Path, **kwargs: Any) -> TaxonomyTree:
        ...
        if file_format == "gtdb_tk_summary":
            next_tax_id = self._parse_gtdb_tk_summary(
                tree, lines, name_to_id, processed_genomes, next_tax_id
            )
        elif file_format == "standard_taxonomy":
            next_tax_id = self._parse_standard_taxonomy(
                tree, lines, name_to_id, processed_genomes, next_tax_id
            )
        else:
        ...
        logger.info(
            f"Parsed GTDB file: {tree.node_count} nodes, {len(processed_genomes)} genomes"
        )
        return tree
    ...
    def _detect_file_format(self, file_path: Path) -> str:
        ...
        if re.match(r"^.*\.ar\d+\.summary\.tsv$", filename) or re.match(
            r"^.*\.bac\d+\.summary\.tsv$", filename
        ):
            return "gtdb_tk_summary"

        if re.match(r"^ar\d+_taxonomy_r\d+\.tsv$", filename) or re.match(
            r"^bac\d+_taxonomy_r\d+\.tsv$", filename
        ):
        ...
        try:
            ...
            if (
                "user_genome" in first_line.lower()
                and "classification" in first_line.lower()
            ):
                return "gtdb_tk_summary"
    ...
    def _parse_gtdb_tk_summary(
        self,
        tree: TaxonomyTree,
        lines: List[str],
        name_to_id: Dict[str, int],
        processed_genomes: Set[str],
        next_tax_id: int,
    ) -> int:
        ...
        for line_num, line in enumerate(lines[data_start_idx:], data_start_idx + 1):
            ...
            try:
                ...
                genome_id = (
                    parts[genome_col]
                    if genome_col < len(parts)
                    else f"genome_{line_num}"
                )
    ...
    def _add_taxonomy_to_tree(
        ...
        for level_name, rank in taxonomy_levels:
            if level_name in name_to_id:
                ...
                node = TaxonomyNode(
                    tax_id=current_id, name=level_name, rank=rank, parent_id=parent_id
                )
                tree.add_node(node)
    ...
    def _parse_gtdb_taxonomy_string(
