        ...
        with self._cache_lock:
            ...
            if node is not None:
                ...
                entry = NodeCacheEntry(
                    node=node, last_accessed=time.time(), access_count=1
                )
        ...
        while current_id is not None:
            if current_id in visited:
                raise ValidationError(
                    f"Circular reference detected at node {current_id}"
                )
...
class StreamingTaxonomyTree:
    ...
    def estimate_memory_usage(self, max_concurrent_nodes: int = 100) -> Dict[str, int]:
        ...
        sample_node = self.node_provider.get_node(
            next(iter(self.node_provider.get_all_node_ids()))
        )
...
class MemoryOptimizedOperations:
    ...
    def batch_lca(
        self, node_pairs: List[tuple[int, int]], batch_size: int = 50
    ) -> Iterator[tuple[tuple[int, int], Optional[TaxonomyNode]]]:
    ...
    def streaming_distance_matrix(
        self, tax_ids: List[int]
        ...
        for i, tax_id1 in enumerate(tax_ids):
            for tax_id2 in tax_ids[i + 1 :]:
                ...
                if lca is None:
                    ...
                    distance = (
                        (lca_pos1 + lca_pos2)
                        if lca_pos1 is not None and lca_pos2 is not None
                        else None
                    )
...
def create_memory_optimized_tree(
    database_path: str, cache_size: int = 1000
) -> LazyTaxonomyTree:
