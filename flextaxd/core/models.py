...
class TaxonomyTree:
    ...
    def extract_subtree(self, root_tax_id: int, preserve_tax_ids: bool = True) -> 'TaxonomyTree':
        ...
        def get_depth(tax_id: int) -> int:
            ...
            while current and current.parent_id is not None and current.parent_id in subtree_nodes:
                depth += 1
    ...
    def graft_subtree(self, subtree: 'TaxonomyTree', attachment_point: int, 
                     id_offset: Optional[int] = None) -> Dict[int, int]:
    ...
    def merge_trees(self, other_tree: 'TaxonomyTree', merge_strategy: str = 'graft_at_root') -> Dict[int, int]:
        ...
        if merge_strategy == 'graft_at_root':
            ...
            for other_node in other_tree:
                for our_node in self:
                    if (other_node.name == our_node.name and 
                        other_node.rank == our_node.rank):
            ...
            for other_node in other_tree:
                if other_node.tax_id not in id_mapping:
                    ...
                    if other_node.parent_id is not None and other_node.parent_id in id_mapping:
